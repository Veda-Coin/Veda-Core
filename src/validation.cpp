...
#if defined(NDEBUG)
# error "Veda Core cannot be compiled without assertions."
#endif
...
/*
    if (nPrevHeight < 5465) {
        // Early ages...
        // 1111/((x+1)^2)
        dSubsidyBase = (1111.0 / (pow((dDiff+1.0),2.0)));
        if(dSubsidyBase > 500) dSubsidyBase = 500;
        else if(dSubsidyBase < 1) dSubsidyBase = 1;
    } else if (nPrevHeight < 17000 || (dDiff <= 75 && nPrevHeight < 24000)) {
        // CPU mining era
        // 11111/(((x+51)/6)^2)
        dSubsidyBase = (11111.0 / (pow((dDiff+51.0)/6.0,2.0)));
        if(dSubsidyBase > 500) dSubsidyBase = 500;
        else if(dSubsidyBase < 25) dSubsidyBase = 25;
    } else {
        // GPU/ASIC mining era
        // 2222222/(((x+2600)/9)^2)
        dSubsidyBase = (2222222.0 / (pow((dDiff+2600.0)/9.0,2.0)));
        if(dSubsidyBase > 25) dSubsidyBase = 25;
        else if(dSubsidyBase < 5) dSubsidyBase = 5;
    }
*/
    if (nPrevHeight < 1)
	dSubsidyBase = 200000;
    else if (nPrevHeight < 14400)
	dSubsidyBase = 0.01;
    else if (nPrevHeight < 33600)
	dSubsidyBase = 30;
    else if (nPrevHeight < 52800)
	dSubsidyBase = 25;
    else if (nPrevHeight < 81600)
	dSubsidyBase = 22;
    else if (nPrevHeight < 110400)
	dSubsidyBase = 20;
    else if (nPrevHeight < 139200)
	dSubsidyBase = 18;
    else if (nPrevHeight < 489600)
	dSubsidyBase = 17;
    else if (nPrevHeight < 840000)
	dSubsidyBase = 15.1;
    else if (nPrevHeight < 1190400)
	dSubsidyBase = 10.71;
    else if (nPrevHeight < 1540800)
	dSubsidyBase = 5.355;
    else if (nPrevHeight < 1891200)
	dSubsidyBase = 2.6775;
    else if (nPrevHeight < 2241600)
	dSubsidyBase = 1.33875;
    else if (nPrevHeight < 2592000)
	dSubsidyBase = 0.669375;
    else if (nPrevHeight < 2942400)
    	dSubsidyBase = 0.3346875;
    else if (nPrevHeight < 3292800)
       	dSubsidyBase = 0.16734375;
    else if (nPrevHeight < 3643200)
       	dSubsidyBase = 0.083671875;
    else if (nPrevHeight < 3993600)
      	dSubsidyBase = 0.0418359375;
    else if (nPrevHeight < 4344000)
       	dSubsidyBase = 0.0209179688;
    else 
    	dSubsidyBase = 0.0104589844;

    // LogPrintf("height %u diff %4.2f reward %d\n", nPrevHeight, dDiff, dSubsidyBase);
    CAmount nSubsidy = (CAmount)(dSubsidyBase * COIN);
...
{
    CAmount ret = blockValue * 90 / 100; // start at 90%
...
*/
    ...
    if(nHeight > 6307200) ret = blockValue; //mainnet: 6307200 - 100% after 1 year
    return ret;
}
...
bool IsInitialBlockDownload()
{
    ...
    if (fImporting || fReindex)
        return true;
//    printf("IsInitialBlockDownload--fImporting: %d, fReindex: %d..................................\n", fImporting, fReindex);
    LOCK(cs_main);
    const CChainParams& chainParams = Params();
    if (chainActive.Tip() == NULL)
        return true;
//	printf("IsInitialBlockDownload--chainActive.Tip() is not NULL.....................\n");
//	printf("IsInitialBlockDownload--chainActive.Tip()->nChainWork: %ld, nMininmumChainWork: %ld.................\n", chainActive.Tip()->nChainWork, UintToArith256(chainParams.GetConsensus().nMinimumChainWork));
    if (chainActive.Tip()->nChainWork < UintToArith256(chainParams.GetConsensus().nMinimumChainWork))
        return true;

//	printf("IsInitialBlockDownload--chainActive.Tip()->GetBlockTime(): %ld, GetTime : %ld,  chainParams.MaxTipAge(): %ld.........\n", chainActive.Tip()->GetBlockTime(), GetTime(), chainParams.MaxTipAge());
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // VEDA : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(VEDA): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(VEDA): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // VEDA : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(VEDA): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(VEDA): spork is off, skipping transaction locking checks\n");
    }
