...
void CMasternodeSync::SwitchToNextAsset(CConnman& connman) {
	...
	case (MASTERNODE_SYNC_WAITING):
		ClearFulfilledRequests(connman);
		LogPrintf(
				"CMasternodeSync::SwitchToNextAsset -- Completed %s in %llds\n",
...
void CMasternodeSync::ProcessMessage(CNode* pfrom, std::string& strCommand,
		CDataStream& vRecv) {
...
void CMasternodeSync::ClearFulfilledRequests(CConnman& connman) {
	...
	connman.ForEachNode(CConnman::AllNodes,
			[](CNode* pnode) {
...
void CMasternodeSync::ProcessTick(CConnman& connman) {
	static int nTick = 0;
	if (nTick++ % MASTERNODE_SYNC_TICK_SECONDS != 0)
		return;
	...
	double nSyncProgress = double(
			nRequestedMasternodeAttempt + (nRequestedMasternodeAssets - 1) * 8)
			/ (8 * 4);
	...
	BOOST_FOREACH(CNode * pnode, vNodesCopy)
	{
		...
		if (Params().NetworkIDString() == CBaseChainParams::REGTEST) {
			if (nRequestedMasternodeAttempt <= 2) {
				connman.PushMessageWithVersion(pnode, INIT_PROTO_VERSION,
						NetMsgType::GETSPORKS); //get current network sporks
			} else if (nRequestedMasternodeAttempt < 4) {
				mnodeman.DsegUpdate(pnode, connman);
			} else if (nRequestedMasternodeAttempt < 6) {
				int nMnCount = mnodeman.CountMasternodes();
				connman.PushMessage(pnode, NetMsgType::MASTERNODEPAYMENTSYNC,
						nMnCount); //sync payment votes
		...
		// NORMAL NETWORK MODE - TESTNET/MAINNET
		{
			...
			if (!netfulfilledman.HasFulfilledRequest(pnode->addr,
					"spork-sync")) {
				...
				LogPrintf(
						"CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d -- requesting sporks from peer %d\n",
			...
			if (nRequestedMasternodeAssets == MASTERNODE_SYNC_LIST) {
				LogPrint("masternode",
						"CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d nTimeLastBumped %lld GetTime() %lld diff %lld\n",
						nTick, nRequestedMasternodeAssets, nTimeLastBumped,
						GetTime(), GetTime() - nTimeLastBumped);
				// check for timeout first
				if (GetTime() - nTimeLastBumped
						> MASTERNODE_SYNC_TIMEOUT_SECONDS) {
					LogPrintf(
							"CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d -- timeout\n",
							nTick, nRequestedMasternodeAssets);
					if (nRequestedMasternodeAttempt == 0) {
						LogPrintf(
								"CMasternodeSync::ProcessTick -- ERROR: failed to sync %s\n",
								GetAssetName());
				...
				if (netfulfilledman.HasFulfilledRequest(pnode->addr,
					...
					continue;
				netfulfilledman.AddFulfilledRequest(pnode->addr,
						"masternode-list-sync");
			...
			if (nRequestedMasternodeAssets == MASTERNODE_SYNC_MNW) {
				...
				if (nRequestedMasternodeAttempt > 1
						&& mnpayments.IsEnoughData()) {
					LogPrintf(
							"CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d -- found enough data\n",
							nTick, nRequestedMasternodeAssets);
					SwitchToNextAsset(connman);
					connman.ReleaseNodeVector(vNodesCopy);
					return;
			...
			if (nRequestedMasternodeAssets == MASTERNODE_SYNC_GOVERNANCE) {
				...
				if (GetTime() - nTimeLastBumped
					...
					if (nRequestedMasternodeAttempt == 0) {
						LogPrintf(
								"CMasternodeSync::ProcessTick -- WARNING: failed to sync %s\n",
								GetAssetName());
				...
				if (netfulfilledman.HasFulfilledRequest(pnode->addr,
					...
					if (nObjsLeftToAsk == 0) {
						...
						if (nLastTick == nTick)
							continue;
						if (GetTime() - nTimeNoObjectsLeft
								> MASTERNODE_SYNC_TIMEOUT_SECONDS
								&& governance.GetVoteCount() - nLastVotes
										< std::max(int(0.0001 * nLastVotes),
							...
							LogPrintf(
									"CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d -- asked for all objects, nothing to do\n",
									nTick, nRequestedMasternodeAssets);
...
void CMasternodeSync::NotifyHeaderTip(const CBlockIndex *pindexNew,
		bool fInitialDownload, CConnman& connman) {
	LogPrint("mnsync",
...
void CMasternodeSync::UpdatedBlockTip(const CBlockIndex *pindexNew,
	...
	bool fReachedBestHeaderNew = pindexNew->GetBlockHash()
			== pindexBestHeader->GetBlockHash();
	...
	LogPrint("mnsync",
			...
			pindexNew->nHeight, pindexBestHeader->nHeight, fInitialDownload,
			fReachedBestHeader);
