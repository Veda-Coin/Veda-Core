...
int64_t UpdateTime(CBlockHeader* pblock,
	...
	int64_t nNewTime = std::max(pindexPrev->GetMedianTimePast() + 1,
			GetAdjustedTime());
...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams,
	...
	nBlockMaxSize = std::max((unsigned int) 1000,
			std::min((unsigned int) (MaxBlockSize(fDIP0001ActiveAtTip) - 1000),
					nBlockMaxSize));
	...
	std::priority_queue<CTxMemPool::txiter, std::vector<CTxMemPool::txiter>,
...

		{
			...
			if (fPriorityBlock) {
				...
				for (CTxMemPool::indexed_transaction_set::iterator mi =
						mempool.mapTx.begin(); mi != mempool.mapTx.end();
						++mi) {
					double dPriority = mi->GetPriority(nHeight);
					CAmount dummy;
					mempool.ApplyDeltas(mi->GetTx().GetHash(), dPriority,
							dummy);
			...
			CTxMemPool::indexed_transaction_set::nth_index<3>::type::iterator mi =
					mempool.mapTx.get<3>().begin();
			...
			while (mi != mempool.mapTx.get<3>().end() || !clearedTxs.empty()) {
				...
				if (fPrintPriority) {
					...
					LogPrintf("priority %.1f fee %s txid %s\n", dPriority,
							CFeeRate(iter->GetModifiedFee(), nTxSize).ToString(),
							tx.GetHash().ToString());
				}
	...
	CAmount blockReward = nFees
			+ GetBlockSubsidy(pindexPrev->nBits, pindexPrev->nHeight,
					Params().GetConsensus());
...
// ***TODO*** ScanHash is not yet used in Veda
//
...
void static BitcoinMiner(const CChainParams& chainparams, CConnman& connman) {
	LogPrintf("VedaMiner -- started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("veda-miner");
	...
	try {
		...
		while (true) {
			if (chainparams.MiningRequiresPeers()) {
				...
				do {
					bool fvNodesEmpty = connman.GetNodeCount(
							CConnman::CONNECTIONS_ALL) == 0;
//		    printf("BitcoinMiner---NodeCount: %d..........\n", connman.GetNodeCount(CConnman::CONNECTIONS_ALL));
...
//		    printf("BitcoinMiner: .........fvNodesEmpty: %d.............IsInitialBlockDownload(): %d...masternodeSync.IsSynced(): %d\n", fvNodesEmpty, IsInitialBlockDownload(), masternodeSync.IsSynced());
			...
			if (!pblocktemplate.get()) {
				LogPrintf(
						"VedaMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
				return;
			}
			...
			LogPrintf(
					"VedaMiner -- Running miner with %u transactions in block (%u bytes)\n",
					pblock->vtx.size(),
			...
			if (pindexPrev->nHeight >= 15237 && pindexPrev->nHeight <= 15350)
				hashTarget = UintToArith256(chainparams.GetConsensus().powLimit);
			else
			...
			while (true) {
				...
				if (UpdateTime(pblock, chainparams.GetConsensus(), pindexPrev)
						< 0)
		...
		LogPrintf("VedaMiner -- terminated\n");
		throw;
	} catch (const std::runtime_error &e) {
		LogPrintf("VedaMiner -- runtime error: %s\n", e.what());
		return;
	}
}
...
void GenerateBitcoins(bool fGenerate, int nThreads,
	...
	for (int i = 0; i < nThreads; i++)
		minerThreads->create_thread(
				boost::bind(&BitcoinMiner, boost::cref(chainparams),
						boost::ref(connman)));
}
